...
/*-----------------------------------------------------------------------------------*/
	...
	$(document).bind('newDeviceSize', function(obj, device){
		if(device.type === 'mobilephone'){
			adjust();
			$('a')
		}else{
			/* PrettyPhoto */
			//$("a[rel^='prettyPhoto']").prettyPhoto({animationSpeed:'slow',theme:'light_square',slideshow:2000, autoplay_slideshow: false, social_tools: false});
			//$("a[rel^='prettyPhoto']").prettyPhoto();
		}
		$('body').removeClass('mobilephone');
		$('body').removeClass('tablet');
		$('body').removeClass('computer');
		$('body').addClass(device.type);
	});
